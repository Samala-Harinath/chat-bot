(function () {
    ...
    const defaultConfig = {
        ...
        domain: null, // Custom domain for API calls
        apiKey: null, // API key for authentication
        speechRecognition: true, // Enable/disable speech recognition
        speechLanguage: 'en-IN' // Speech recognition language
    };
    ...
    // Update API URL based on domain config
    // if (config.domain) {
    //     config.apiUrl = config.domain + `hdfcmf_generate_response?userchatuid=62648dee-2f8e-4aa0-8266-99ec0e8b0301&questionuid=527bee81-20f3-464c-9184-98f912159576&username=hsamala&question=Are%20alternative%20treatments%20like%20Ayurveda%20and%20Homeopathy%20covered%20under%20HDFC%20ERGO%20health%20insurance%20policies?&domain=INSURANCE&language=English&rerank=T&userrole=Administrator&llmmodel=LLAMA3`;
    // } else if (config.apiUrl.startsWith('/')) {
    ...
    const speechSupported = SpeechRecognition && config.speechRecognition;
    ...
    class EmbeddableChatbot {
        constructor(config) {
            ...
            this.selectedLanguage = this.config.speechLanguage;
        ...
        createStyles() {
          ...
          .mic-btn {
            background: ${speechSupported ? '#10b981' : '#9ca3af'};
            color: white;
            border: none;
            border-radius: 50%;
            width: 40px;
          ...
          .mic-btn:hover {
            transform: ${speechSupported ? 'scale(1.05)' : 'none'};
          }
        ...
        `;
            ...
            this.languageSelect = container.querySelector('#languageSelect');
        }
        ...
        bindEvents() {
            ...
            if (this.micBtn && speechSupported) {
                this.micBtn.addEventListener('click', () => this.toggleSpeechRecognition());
            }

            if (this.languageSelect && speechSupported) {
        ...
        async sendMessage() {
            ...
            console.log("userMessage",this.messages);
            ...
            try {
                this.config.apiUrl = this.config.domain + `hdfcmf_generate_response?userchatuid=${uuidv4()}&questionuid=${uuidv4()}&username=hsamala&question=${text}&domain=INSURANCE&language=${this.selectedLanguage}&rerank=T&userrole=Administrator&llmmodel=LLAMA3`;
                // const headers = {
